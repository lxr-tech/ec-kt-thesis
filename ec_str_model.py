...
type_tot = {'array': 845, 'hash-table': 281, 'math': 289, 'string': 417,
            'dynamic-programming': 303, 'greedy': 189, 'sorting': 193, 'depth-first-search': 175}
type_num = {'array': 0, 'hash-table': 0, 'math': 0, 'string': 0,
            'dynamic-programming': 0, 'greedy': 0, 'sorting': 0, 'depth-first-search': 0}
...
def data_split(data, name_dict, test_rate=0.2):
    ...
    for i, problem in enumerate(data):
        ...
        for tag in problem['topicTags']:
            ...
            type_num[tag] += 1
            flag &= type_tot[tag] * (1 - test_rate) > type_num[tag]
        if flag:
            train.append(term)
        else:
            test.append(term)
