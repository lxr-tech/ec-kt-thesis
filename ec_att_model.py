...
class EC(nn.Module):
    ...
    def __init__(self, inp_size, mid_size, num_words, num_types=8, num_layers=1, weight=None):
        ...
        self.k_net = nn.Linear(in_features=inp_size, out_features=num_types, bias=False).cuda()
        self.q_net = nn.TransformerEncoderLayer(d_model=num_types, nhead=num_types, activation='gelu').cuda()
    ...
    def forward(self, x):
        ...
        x = self.k_net(x)
        # x.shape = (batch, length, type)
        ques = self.q_net(x)
        # ques.shape = (batch, length, type)
        pred = torch.max(ques, dim=1)[0].squeeze(1)
