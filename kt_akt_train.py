...
class Trainer(object):
    ...
    def train_func(self, batch_data):
        ...
        for batch in batch_data:
            ...
            y = torch.sign(torch.sum(qa[:, :, self.n_question:], dim=-1))
        ...
        for lineID, line in enumerate(f_data):
            ...
            if lineID % 4 == 1:
                ...
                if len(q_seq) > 0:
                    ...
                    y = torch.sign(torch.sum(a_seq[:, :, self.n_question:], dim=-1))
                    pred = torch.round(self.model(q_seq, a_seq, p_seq).squeeze(-1))
                    print(L, file=t_data)
