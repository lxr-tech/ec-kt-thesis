...
class Trainer(object):
    ...
    def __init__(self, net, epoch_num, lr_ec, lr_kt, bs_ec, bs_kt, l2_ec, l2_kt, max_grad_norm=-1):
        self.net = net
        ...
        self.opt_ec = optim.Adam(net.ec_net.parameters(), lr=lr_ec, weight_decay=l2_ec)
        self.opt_kt = optim.Adam(net.parameters(), lr=lr_kt, weight_decay=l2_kt)
        self.loss_kt = nn.MSELoss(reduction='sum')
        self.loss_ec = F.cosine_similarity
        self.n_question = self.net.n_question

    def model_save(self, epoch, n_ec, n_kt, path):
        state = {'kt_net': self.net.state_dict(),
                 'ec_net': self.net.ec_net.state_dict(),
    ...
    def kt_func(self, batch_data):
        ...
        for i, batch in enumerate(batch_data):
            a, pid = batch
            a, pid = a.cuda(), pid.cuda()
            y = 2 * torch.sign(torch.sum(a[:, :, self.n_question:], dim=-1)) - 1
            pred = self.net(a, pid).squeeze(-1)
            self.net.zero_grad()
            loss = self.loss_kt(pred, y)
            loss_tra += loss.item()
            loss.backward()
    ...
    def ec_func(self, batch_data):
        ...
        for i, batch in enumerate(batch_data):
            x, y = batch
            y = y.cuda()
            pred = self.net.ec_net(x, ec=True).cuda()
            loss = 1 - self.loss_ec(pred, y).cuda()
            loss = torch.sum(loss, dim=0)
            loss_tra += loss.item()
            self.net.ec_net.zero_grad()
            loss.backward()
            if self.max_grad_norm > 0:
                torch.nn.utils.clip_grad_norm_(self.net.ec_net.parameters(), max_norm=self.max_grad_norm)
            self.opt_ec.step()
        return loss_tra
    ...
    def ec_eval(self, batch_data):
        ...
        for i, batch in enumerate(batch_data):
            ...
            acc1.append(acc.item())
            acc = torch.mean(
                torch.tensor([pred[k][key].equal(y[k][key]) for k in range(pred.shape[0])], dtype=torch.float))
            acc2.append(acc.item())
            acc = torch.mean((torch.tensor(pred == y, dtype=torch.float)))
            acc3.append(acc.item())
            hit = torch.mean(torch.tensor(
                [(torch.sum(pred[k] * y[k]) + 1e-5) / (torch.sum(pred[k]) + 1e-5) for k in range(pred.shape[0])],
                dtype=torch.float))
            hit1.append(hit.item())
            hit = torch.mean(torch.tensor(
                [(torch.sum(pred[k] * y[k]) + 1e-5) / (torch.sum(y[k]) + 1e-5) for k in range(pred.shape[0])],
                dtype=torch.float))
    ...
    def ec_valid(self, glove, path):
        ...
        for item in glove.data:
            ...
            item = torch.LongTensor([item['questionId']])
            pred = torch.round(self.net.ec_net(item, ec=True))
        ...
        for lineID, line in enumerate(f_data):
            ...
            if lineID % 4 == 1:
                ...
                if len(p_seq) > 0:
                    ...
                    pred = self.net(a_seq, p_seq).squeeze(-1)
                    pred = torch.round(pred)
                    print(L, file=t_data)
